...
public:
    ...
    struct Edge {
        ...
        NodeID fromID() const { return from->getID(); }
        NodeID toID() const { return to->getID(); }
    ...
    using EdgeContainer = std::pmr::vector<Edge*>;
    using NodeContainer = std::pmr::vector<Node*>;
    ...
    void export_graphviz(std::ostream& out, const char* name = nullptr);
...
inline DependencyGraph::Edge::Edge(DependencyGraph& graph,
        ...
        : from(from), to(to) {
    assert(graph.mNodes[this->fromID()] == from);
    assert(graph.mNodes[this->toID()] == to);
    graph.link(this);
}
